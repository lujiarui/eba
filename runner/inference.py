...
class InferenceRunner(object):
    ...
    @torch.no_grad()
    def predict(self, data: Mapping[str, Mapping[str, Any]], dump_embeddings=False) -> dict[str, torch.Tensor]:
        eval_precision = {
            "fp32": torch.float32,
            "bf16": torch.bfloat16,
            "fp16": torch.float16,
        ...
        with enable_amp:
            if dump_embeddings:
                print(f"Generating embeddings for {data['sample_name']} | N_cycle: {self.model.N_cycle}")
...
def infer_predict(runner: InferenceRunner, configs: Any, dump_embeddings=False) -> None:
    # Data
    ...
    for seed in configs.seeds:
        ...
        for batch in dataloader:
            try:
                ...
                prediction = runner.predict(data, dump_embeddings=dump_embeddings)
                runner.dumper.dump(
                    dataset_name="",
                    pdb_id=sample_name,
                    seed=seed,
...
def main(configs: Any) -> None:
    ...
    infer_predict(runner, configs, dump_embeddings=False)
