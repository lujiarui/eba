...
class AF3Trainer(object):
    ...
    def init_model(self):
        ...
        # Set the diffusion module reference
        self.diffusion_module_ref = None
    ...
    def model_forward(self, batch: dict, mode: str = "train") -> tuple[dict, dict]:
        assert mode in ["train", "eval", "encode", "predict"]
        if mode == "encode":
            log_dict = None
            ...
            batch["pred_dict"], batch["label_dict"], log_dict = self.model(
                ...
                dpo_training=self.configs.dpo_training,
                diffusion_module_ref=self.diffusion_module_ref,
            )
    ...
    @torch.no_grad()
    def predict(self, ema_suffix: str = "", mode: str = "eval"):
        # Init Metric Aggregator
        eval_precision = {
            "fp32": torch.float32,
            "bf16": torch.bfloat16,
        ...
        for test_name, test_dl in self.test_dls.items():
            ...
            for index, batch in enumerate(tqdm(test_dl)):
                ...
                if index + 1 == total_batch_num and DIST_WRAPPER.world_size > 1:
                    ...
                    all_data_ids = DIST_WRAPPER.all_gather_object(evaluated_pids)
                    dedup_ids = set(sum(all_data_ids, []))
                    if pc_name in dedup_ids:
                        print(
                            f"Rank {DIST_WRAPPER.rank}: Drop data_id {pc_name} as it is already evaluated."
                        )
                        break
                evaluated_pids.append(pc_name)

                with enable_amp:
                    # Model forward
                    batch, _ = self.model_forward(batch, mode=mode)
                    prediction = batch["pred_dict"]
                    # Save prediction
                    self.dumper.dump(
    ...
    def run(self):
        ...
        if self.configs.dpo_training:
            # initialize the diffusion module as reference model
            print(">>> [protenix.py] debug: instantiate diffusion_module_ref")
            self.diffusion_module_ref = DiffusionModule(**self.configs.model.diffusion_module).to(self.device)
            self.diffusion_module_ref.eval()
            self.diffusion_module_ref.load_state_dict(self.raw_model.diffusion_module.state_dict())
            for param in self.diffusion_module_ref.parameters():
    ...
    def dump_embeddings(self):
        ...
        for test_name, test_dl in self.test_dls.items():
            ...
            for index, batch in enumerate(tqdm(test_dl, desc="iterating target embeddings")):
                ...
                pc_name = '_'.join(pid.split("_")[:2])
                if os.path.exists(f"{self.dump_dir}/{pc_name}.pt"):
                    continue

                batch = to_device(batch, self.device)
                evaluated_pc_names.append(pc_name)
                with enable_amp:
                    # Model forward
                    ...
                    torch.save(output_emb, f"{self.dump_dir}/{pc_name}.pt")
...
def main():
    ...
    if configs.get("dump_embeddings", False):
        ...
        return
    elif configs.get("predict_only", False):
        print("Predict only mode on")
        trainer.predict(mode="predict")
        return
    else:
        trainer.run()
