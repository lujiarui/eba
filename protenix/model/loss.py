...
class ProtenixLoss(nn.Module):
    ...
    def __init__(self, configs) -> None:
        ...
        self.skip_confidence_and_permutation = self.configs.skip_confidence_and_permutation
        ...
        self.dpo_enabled =  configs.loss.diffusion.get("dpo_enabled", False)
        self.beta_dpo = configs.loss.diffusion.get("beta_dpo", 0)
        self.linear_ew = configs.loss.diffusion.get("linear_ew", False)
        self.eba_enabled = configs.loss.diffusion.get("eba_enabled", False)
        self.beta_eba = configs.loss.diffusion.get("beta_eba", 0.4)
        self.eba_with_ref = configs.loss.diffusion.get("eba_with_ref", False)
        self.norm_by_len = configs.loss.diffusion.get("norm_by_len", False)

    def calculate_label(
        self,
        feat_dict: dict[str, Any],
        label_dict: dict[str, Any],
        dpo_sample_type: Optional[str] = None,
    ) -> dict[str, Any]:
        ...
        Returns:
            label_dict (dict): with the following updates:
                ...
                distance_mask (torch.Tensor): atom-atom mask indicating whether true distance exists.
                    [..., N_atom, N_atom]
        """
        # if dpo_sample_type is not None:
            # assert dpo_sample_type in ("w", "l")    # label
        coordinate_key = "coordinate" + (f"_{dpo_sample_type}" if dpo_sample_type else "")
        distance_key = "distance" + (f"_{dpo_sample_type}" if dpo_sample_type else "")
        ...
        distance = (
            cdist(label_dict[coordinate_key], label_dict[coordinate_key]) * distance_mask
        ).to(
            label_dict[coordinate_key].dtype
        )  # [..., N_atom, N_atom]
        ...
        if not self.configs.loss_metrics_sparse_enable:
            label_dict[distance_key] = distance
    ...
    def calculate_prediction(
        ...
        """
        # if dpo_sample_type is not None:
            # assert dpo_sample_type in ("w", "w_ref", "l", "l_ref")

        coordinate_key = "coordinate" + (f"_{dpo_sample_type}" if dpo_sample_type else "")
        distance_key = "distance" + (f"_{dpo_sample_type}" if dpo_sample_type else "")

        if not self.configs.loss_metrics_sparse_enable:
            pred_dict[distance_key] = torch.cdist(
                pred_dict[coordinate_key], pred_dict[coordinate_key]
            ).to(
                pred_dict[coordinate_key].dtype
            )  # [..., N_atom, N_atom]
        return pred_dict
    ...
    def calculate_losses(
        ...
        if self.configs.train_confidence_only and mode == "train":
            ...
            coordinate_key = "coordinate" + (f"_{dpo_sample_type}" if dpo_sample_type else "")
            distance_key = "distance" + (f"_{dpo_sample_type}" if dpo_sample_type else "")
            coordinate_label_key = coordinate_key if dpo_sample_type is None or "_ref" not in dpo_sample_type else coordinate_key[:-4]
            distance_label_key = distance_key if dpo_sample_type is None or "_ref" not in dpo_sample_type else distance_key[:-4]
                ...
                loss_fns.update(
                    {
                        "smooth_lddt_loss": lambda: self.smooth_lddt_loss.dense_forward(
                            pred_coordinate=pred_dict[coordinate_key],
                            true_coordinate=label_dict[coordinate_label_key],
                ...
                loss_fns.update(
                    {
                        "smooth_lddt_loss": lambda: self.smooth_lddt_loss(
                            pred_distance=pred_dict[distance_key],
                            true_distance=label_dict[distance_label_key],
        ...
        if has_valid_resolution:
            has_valid_resolution = torch.tensor(
                [1.0],
                dtype=label_dict[coordinate_label_key].dtype,
                device=label_dict[coordinate_label_key].device,
            )
        else:
            has_valid_resolution = torch.tensor(
                [0.0],
    ...
    def forward(
        ...
        if not self.dpo_enabled: # get multiple matching diff terms and calculate dpo loss
            # Pre-computations
            with torch.no_grad():
                label_dict = self.calculate_label(feat_dict, label_dict)
     
            dpo_sample_type, coordinate_key = None, "coordinate"
            ### raw subroutine ###
            ...
            if diffusion_chunk_size <= 0:
                ...
                if coordinate_key in pred_dict:
                    N_sample = pred_dict[coordinate_key].shape[-3]
                ...
                for i in range(no_chunks):
                    ...
                    for key, value in pred_dict.items():
                        if key in [coordinate_key] and mode == "train":
                            ...
                            key in [coordinate_key, "plddt", "pae", "pde", "resolved"]
                            and mode != "train"
                        ):
                            pred_dict_i[key] = value[
                                i * diffusion_chunk_size : (i + 1) * diffusion_chunk_size,
            ...
            for k in label_dict.keys():
                if k.startswith("energy_"):    # unique suffix
                    dpo_sample_idx_list.append(k.split("_")[-1])

            # Pre-computations
            with torch.no_grad():
                for dpo_sample_idx in dpo_sample_idx_list:  # loop to augment label dict with "distance_idx"
                    label_dict = self.calculate_label(feat_dict, label_dict, dpo_sample_type=dpo_sample_idx)
                # label_dict_w = self.calculate_label(feat_dict, label_dict, dpo_sample_type="w")
                # label_dict_l = self.calculate_label(feat_dict, label_dict, dpo_sample_type="l")
            dpo_sample_idx_with_ref = dpo_sample_idx_list + [f"{dpo_sample_idx}_ref" for dpo_sample_idx in dpo_sample_idx_list]
            for dpo_sample_type in dpo_sample_idx_with_ref:
                coordinate_key = "coordinate" + f"_{dpo_sample_idx}"
                ### raw subroutine ###
                pred_dict = self.calculate_prediction(pred_dict, dpo_sample_type=dpo_sample_type)
                if diffusion_chunk_size <= 0:
                    # Calculate losses
                    cum_loss, losses = self.calculate_losses(
                        feat_dict=feat_dict,
                        ...
                        label_dict=label_dict,
                        mode=mode,
                        dpo_sample_type=dpo_sample_type
                    )
                else:
                    if coordinate_key in pred_dict:
                        N_sample = pred_dict[coordinate_key].shape[-3]
                    ...
                    for i in range(no_chunks):
                        ...
                        for key, value in losses_i.items():
                            if key in losses:
                                ...
                                losses[key] = value * cur_sample_num
                    cum_loss /= N_sample    # visable batch size == 1
                    for key in losses.keys():
                        losses[key] /= N_sample
                
                # record by part
                all_losses.update({key + f"_{dpo_sample_type}": value for key, value in losses.items()})
                # dpo loss components
                sample_type_to_loss[dpo_sample_type] = cum_loss
            
            if not self.eba_enabled:
                # gather each sample loss and calculate dpo loss
                # https://github.com/SalesforceAIResearch/DiffusionDPO/blob/main/train.py#L1142C21-L1145C65
                
                # infer the win "w" and lose "l" sample type
                assert len(dpo_sample_idx_list) == 2, f"Currently only support 2 sample types for DPO loss calculation, got {dpo_sample_idx_list}"
                idx1, idx2 = dpo_sample_idx_list
                win_idx = idx1 if label_dict[f"energy_{idx1}"] < label_dict[f"energy_{idx2}"] else idx2
                lose_idx = idx1 if label_dict[f"energy_{idx1}"] >= label_dict[f"energy_{idx2}"] else idx2
                abs_energy_diff = np.abs(label_dict[f"energy_{win_idx}"] - label_dict[f"energy_{lose_idx}"])
                
                # replace key name in all_losses: with "win" and "lose"
                all_losses = {k.replace(f"_{win_idx}", "_win").replace(f"_{lose_idx}", "_lose"): v for k, v in all_losses.items()}
            
                scale_term = -0.5 * self.beta_dpo
                if self.linear_ew:
                    # the larger the energy difference, the larger the scale term -> stronger penalty 
                    scale_term = scale_term * abs_energy_diff

                model_diff = sample_type_to_loss[win_idx] - sample_type_to_loss[lose_idx]
                ref_diff = sample_type_to_loss[f"{win_idx}_ref"] - sample_type_to_loss[f"{lose_idx}_ref"]
                inside_term = scale_term * (model_diff - ref_diff)
                implicit_acc = (inside_term > 0).sum().float() #/ inside_term.size(0)
                cum_loss_dpo = -1 * F.logsigmoid(inside_term).mean()
                
                all_losses["model_win_over_lose"] = model_diff.detach().clone()
                all_losses["dpo_loss_all"] = cum_loss_dpo.detach().clone()
                return cum_loss_dpo, all_losses
            else:
                # use eba loss
                energies = torch.as_tensor(
                    [label_dict[f"energy_{dpo_sample_idx}"] for dpo_sample_idx in dpo_sample_idx_list], 
                dtype=label_dict["distance_mask"].dtype, device=label_dict["distance_mask"].device)
                energies = - energies * self.beta_eba
                if self.norm_by_len:    # normalize by the number of residues in the protein
                    L = feat_dict["token_index"].shape[0]
                    assert L > 1, f"Invalid protein length: {L} | shape: {feat_dict['token_index'].shape}"
                    energies = energies / (L ** 0.5)
                softmax_energies = F.softmax(energies)
                # print(f"debug | energies: {energies} | softmax_energies: {softmax_energies}")
                tmp_all_losses = {}
                for i, idx in enumerate(dpo_sample_idx_with_ref):
                    # replace loss key name in all_losses: idx -> i
                    tmp_all_losses.update(
                        {k.replace(f"_{idx}", f"_{i}"):v for k,v in all_losses.items() if k.endswith(idx)}
                    )
                all_losses = tmp_all_losses
                sample_neg_diff_loss = - torch.stack([sample_type_to_loss[dpo_sample_idx] for dpo_sample_idx in dpo_sample_idx_list])
                if self.eba_with_ref:
                    # use advantage over ref sample
                    sample_neg_diff_loss_ref = - torch.stack([sample_type_to_loss[f"{dpo_sample_idx}_ref"] for dpo_sample_idx in dpo_sample_idx_list])
                    sample_neg_diff_loss = sample_neg_diff_loss - sample_neg_diff_loss_ref
                logsoftmax_neg_diff = F.log_softmax(self.beta_dpo * sample_neg_diff_loss)
                cum_loss_eba = - torch.sum(softmax_energies * logsoftmax_neg_diff)
                all_losses["eba_loss_all"] = cum_loss_eba.detach().clone()
                all_losses["eba_loss_flat"] = (- torch.sum(logsoftmax_neg_diff)).detach().clone()
                return cum_loss_eba, all_losses