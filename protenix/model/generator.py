...
def dpo_diffusion_training(
    ...
    N_sample: int = 1,
    diffusion_chunk_size: Optional[int] = None,
    denoise_net_ref: Optional[Callable] = None,
) -> tuple[torch.Tensor, ...]:
    # call twice for each net to get two sets of denoised coordinates
    x_gt_w = label_dict["coordinate_w"]
    x_gt_l = label_dict["coordinate_l"]

    batch_size_shape = label_dict["coordinate_w"].shape[:-2]
    device = label_dict["coordinate_w"].device
    dtype = label_dict["coordinate_w"].dtype
    
    # Areate N_sample versions of the input structure by randomly rotating and translating
    x_gt_augment_w = centre_random_augmentation(
        x_input_coords=x_gt_w,
        N_sample=N_sample,
        mask=label_dict["coordinate_mask_w"],
    ).to(
        dtype
    )  # [..., N_sample, N_atom, 3]

    x_gt_augment_l = centre_random_augmentation(
        x_input_coords=x_gt_l,
        N_sample=N_sample,
        mask=label_dict["coordinate_mask_l"],
    ).to(
        dtype
    )  # [..., N_sample, N_atom, 3]

    # sigma is shared between win-lose pair
    sigma = noise_sampler(size=(*batch_size_shape, N_sample), device=device).to(dtype)
    # noise: [..., N_sample, N_atom, 3]
    noise = torch.randn_like(x_gt_augment_w, dtype=dtype) * sigma[..., None, None]

    def local_fn(_net, _x_gt_augment):
        # Get denoising outputs [..., N_sample, N_atom, 3]
        if diffusion_chunk_size is None:
            x_denoised = _net(
                x_noisy=_x_gt_augment + noise,
                t_hat_noise_level=sigma,
                input_feature_dict=input_feature_dict,
                s_inputs=s_inputs,
                s_trunk=s_trunk,
    ...
    x_denoised_w = local_fn(denoise_net, x_gt_augment_w)
    x_denoised_w_ref = local_fn(denoise_net_ref, x_gt_augment_w)
    x_denoised_l = local_fn(denoise_net, x_gt_augment_l)
    x_denoised_l_ref = local_fn(denoise_net_ref, x_gt_augment_l)
    ...
    return (
        ...
        {
            "w": x_denoised_w,
            "w_ref": x_denoised_w_ref,
            "l": x_denoised_l,
            "l_ref": x_denoised_l_ref,
        }, 
        sigma,
    )


def eba_diffusion_training(
    noise_sampler: TrainingNoiseSampler,
    denoise_net: Callable,
    label_dict: dict[str, Any],
    input_feature_dict: dict[str, Any],
    ...
    for k, v in label_dict.items():
        sample_key = k.split("_")[-1]   # index
        if k.startswith("coordinate_mask_"):
            x_mask_dict[sample_key] = v
        elif k.startswith("coordinate_") and "mask" not in k:
            x_gt_dict[sample_key] = v
        else:
            pass    # inrelevant keys

    assert len(x_gt_dict) > 1, "Expected at least two sets of coordinates in label_dict."
    example_key = list(x_gt_dict.keys())[0]
    example_coord = x_gt_dict[example_key]

    batch_size_shape = example_coord.shape[:-2]
    device = example_coord.device
    dtype = example_coord.dtype
    
    # Areate N_sample versions of the input structure by randomly rotating and translating
    for k, v in x_gt_dict.items():
        x_gt_augment_dict[k] = centre_random_augmentation(
            x_input_coords=v,
            N_sample=N_sample,
            mask=x_mask_dict[k],
        ).to(
    ...
    sigma = noise_sampler(size=(*batch_size_shape, N_sample), device=device).to(dtype)
    # noise: [..., N_sample, N_atom, 3]
    noise = torch.randn_like(x_gt_augment_dict[example_key], dtype=dtype) * sigma[..., None, None]
    ...
    x_denoised_dict = {}
    for k in x_gt_dict.keys():
        x_denoised_dict[k] = local_fn(denoise_net, x_gt_augment_dict[k])
        x_denoised_dict[f"{k}_ref"] = local_fn(denoise_net_ref, x_gt_augment_dict[k])
