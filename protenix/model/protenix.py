...
from protenix.model.generator import (
    ...
    dpo_diffusion_training,
    eba_diffusion_training,
)
...
class Protenix(nn.Module):
    ...
    def __init__(self, configs) -> None:
        ...
        self.skip_confidence_and_permutation = configs.skip_confidence_and_permutation
    ...
    @torch.no_grad()    # no grad because we freeze the embedder + pairformer 
    def get_pairformer_output(
        ...
        """
        if "s_inputs" in input_feature_dict:
    ...
    def main_inference_loop(
        ...
        for _ in range(N_model_seed):
            pred_dict, log_dict, time_tracker = self._main_inference_loop(
                ...
                skip_confidence=skip_confidence if mode == "predict" else False,
            )
        ...
        if "summary_confidence" not in pred_dicts[0]:
            # only return the coordinate
            # predict mode
            all_pred_dict = {"coordinate": _cat(pred_dicts, "coordinate")}
        else:
            all_pred_dict = {
    ...
    def main_train_loop(
        ...
        inplace_safe: bool = False,
        chunk_size: Optional[int] = None,
        diffusion_module_ref: Optional[DiffusionModule] = None,
    ) -> tuple[dict[str, torch.Tensor], dict[str, Any], dict[str, Any]]:
        """
        ...
        if not self.skip_confidence_and_permutation:
            ...
            # plddt_pred = pae_pred = pde_pred = resolved_pred = None
            pred_dict.update(
                {
                    "plddt": plddt_pred,
                    "pae": pae_pred,
        ...
        diff_train_fn = sample_diffusion_training if diffusion_module_ref is None else eba_diffusion_training
        _, x_denoised, x_noise_level = autocasting_disable_decorator(
            ...
            denoise_net_ref=diffusion_module_ref,
        )
        
        if isinstance(x_denoised, dict):
            for suffix, _x_denoised_coord in x_denoised.items():
                pred_dict.update(
                    {
                        # "distogram": self.distogram_head(z),
                        # [..., N_sample=48, N_atom, 3]: diffusion loss
                        f"coordinate": _x_denoised_coord,
                ...
                pred_dict[f"coordinate_{suffix}"] = pred_dict[f"coordinate"]
        ...
        # FIXME: this could be problematic because we only log once
        log_dict.update(perm_log_dict)
    ...
    def forward(
        ...
        assert mode in ["train", "inference", "eval", "predict"]
        ...
        if mode == "train":
            ...
            if dpo_training: 
                assert diffusion_module_ref is not None
            else:
                assert diffusion_module_ref is None
            ...
            pred_dict, label_dict, log_dict = self.main_train_loop(
                ...
                inplace_safe=inplace_safe,
                chunk_size=chunk_size,
                diffusion_module_ref=diffusion_module_ref,
            )
        elif mode == "inference":
            ...
            if label_dict is not None:
                assert (
                    label_dict["coordinate"].size() == label_full_dict["coordinate"].size()
                )
                label_dict.update(label_full_dict)
