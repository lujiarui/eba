...
class BaseSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.preference_mode = kwargs.get("preference_mode", False)
        
        if self.preference_mode:    # add pc_name for preference retrieval
            self.indices_list["pc_name"] = self.indices_list["pdb_id"].apply(
                lambda x: "_".join(x.split("_")[:2]))
            
            self.annotation_csv = kwargs.get("annotation_csv", None)
            if self.annotation_csv is not None:
                self.annotation_csv = pd.read_csv(self.annotation_csv)
                _fn = lambda x: os.path.basename(x).split(".cif")[0]
                self.annotation_csv["pdb_id"] = self.annotation_csv["name"].apply(_fn)
                self.annotation_csv = self.annotation_csv.set_index("pdb_id")
                # filter indices_list by annotation_csv
                print("Filter indices_list by the annotation CSV file:")
                print(f">>> Before: {len(self.indices_list)}")
                self.indices_list = self.indices_list[self.indices_list["pdb_id"].isin(self.annotation_csv.index)]
                print(f">>> After: {len(self.indices_list)}")

            # reindex as natural index
            self.indices_list = self.indices_list.reset_index(drop=True)

            self.retrieval_k = kwargs.get("retrieval_k", 1)
            print(f"Retrieved mini-batch samples K: {self.retrieval_k}")
    ...
    def __getitem__(self, idx: int):
        ...
        for _ in range(10):
            try:
                data = self.process_one(idx, return_atom_token_array=True)
                if self.preference_mode:
                    ...
                    # retrieve related data items (shared identity)
                    target_pc_name = self._get_sample_indice(idx=idx).pc_name
                    target_indices = self.indices_list[self.indices_list.pc_name == target_pc_name]
                    
                    # alt_idx = random.choice(target_indices.index)
                    # random sample an alt structure other than the current one
                    candidate_idx = np.array([i for i in target_indices.index if i != idx])
                    
                    if len(candidate_idx) < self.retrieval_k:
                        raise ValueError(f"not enough candidates for retrieval: {len(candidate_idx)} < {self.retrieval_k}")
                    
                    # random sample K = self.retrieval_k
                    augment_indices = np.random.choice(candidate_idx, size=self.retrieval_k, replace=False)

                    # alt_idx = random.choice(candidate_idx)    # binary case
                    # append alt structure and weights to it
                    # sample_indice = self.indices_list.iloc[alt_idx]
                    # sample_indice = target_indices.loc[alt_idx] # index by index || iloc: index by row idx
                    data["label_dict"].update(
                        {
                            f"coordinate_{idx}": data["label_dict"]["coordinate"],
                            f"coordinate_mask_{idx}": data["label_dict"]["coordinate_mask"],
                            f"energy_{idx}": data["basic"]["openmm_energy"],
                        }
                    )

                    for i, alt_idx in enumerate(augment_indices):
                        alt_data = self.process_one(alt_idx, return_atom_token_array=True)
                        get_pc_name = lambda x: '_'.join(x["basic"]["pdb_id"].split("_")[:2])
                        assert get_pc_name(data) == get_pc_name(alt_data), f"pc_name should be the same: {get_pc_name(data)} != {get_pc_name(alt_data)}"

                        # # append alt structure and weights to it
                        # if data["basic"]["openmm_energy"] < alt_data["basic"]["openmm_energy"]:
                        #     data_win, data_lose = data, alt_data
                        # else:
                        #     data_win, data_lose = alt_data, data

                        # # win
                        # coordinate_w = data_win["label_dict"]["coordinate"]
                        # coordinate_mask_w = data_win["label_dict"]["coordinate_mask"]
                        # # lose
                        # coordinate_l = data_lose["label_dict"]["coordinate"]
                        # coordinate_mask_l = data_lose["label_dict"]["coordinate_mask"]
                        
                        # data["label_dict"].pop("coordinate")
                        # data["label_dict"].pop("coordinate_mask")
                        
                        data["label_dict"].update(
                            {
                                f"coordinate_{alt_idx}": alt_data["label_dict"]["coordinate"],
                                f"coordinate_mask_{alt_idx}": alt_data["label_dict"]["coordinate_mask"],
                                f"energy_{alt_idx}": alt_data["basic"]["openmm_energy"],
                            }
                        )
                        # data["label_dict"].update(
                        #     {
                        ...
                        #         "coordinate_mask_w": coordinate_mask_w,
                        #         "coordinate_mask_l": coordinate_mask_l,
                        #         "energy_diff": - (data_win["basic"]["openmm_energy"] - data_lose["basic"]["openmm_energy"]),  # > 0
                        #         "energy_w": data_win["basic"]["openmm_energy"],
                        #         "energy_l": data_lose["basic"]["openmm_energy"],
                        #     }
                        # )
                return data
    ...
    def process_one(
        ...
        if self.precomputed_emb_dir is not None:
            ...
            pc_name = '_'.join(pid.split("_")[:2])
            emb = torch.load(self.precomputed_emb_dir / f"{pc_name}.pt", map_location='cpu')
            # crop the embedding
            if selected_token_indices is not None:
                emb = {
                    "s_inputs": emb["s_inputs"][selected_token_indices].detach(),    # (L, D)
                    "s": emb["s"][selected_token_indices].detach(),
                    "z": emb["z"][selected_token_indices][:, selected_token_indices].detach(),
                }
        ...
        # add annotation
        if self.preference_mode:
            # print("### preference mode on ###")
            if "openmm_annotation" in bioassembly_dict:
                data["basic"]["openmm_energy"] = bioassembly_dict["openmm_annotation"]["final_e"]   # initial_e, rmsd
            elif self.annotation_csv is not None:
                pid = data["basic"]["pdb_id"]
                assert pid in self.annotation_csv.index, f"{pid} not in annotation_csv"
                data["basic"]["openmm_energy"] = self.annotation_csv.loc[pid]["final_e"]
            else:
                raise ValueError("openmm_annotation or annotation_csv should be provided in preference mode")
        return data
