...
parser.add_argument('--atlas_dir', type=str, default='~/scratch/data/atlas/eval_data/eval_targets') 
...
def main(name):
    ...
    try:
        ...
        p = gather_cif_and_save_pdb(
            cif_dir=f'{args.pdbdir}/{name}/seed_42/predictions',
            pdb_dir=f'{os.path.dirname(args.pdbdir)}/pdb',
            name=name,
        )
...
if args.pdb_id:
    ...
    pdb_id = [nam for nam in os.listdir(args.pdbdir) if os.path.isdir(f'{args.pdbdir}/{nam}')]
